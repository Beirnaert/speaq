...
vignette: >
  %\VignetteIndexEntry{speaq2 function illustrations}
...
To illustrate the possibilities of speaq2, the spiritual successor to the original speaq package, the wine dataset is used (available form the University of Copenhagen at http://www.models.life.ku.dk/). This dataset is also utilized in the paper so with this vignette you will be able to reproduce the results.
...
The plot above shows clearly what the basis of speaq2 does, convert spectra accurately to peak data and group these peaks. The quality of the grouping can be checked by plotting the silhouette values. this can be done with the `SilhouetR()` function. 

```{r silhouette values,  results = "hide", dpi=100, fig.width=6, fig.height=3.5, out.width = figwidth.out}
library(ggplot2)
...
```{r PCA, dpi=150, fig.width=5, fig.height=3, out.width = figwidth.out}
library(stats)
...
```{r r significant features plots , dpi=200, fig.width=6, fig.height=8, fig.keep = "all", tidy = FALSE, warnings = FALSE, out.width = figwidth.out}
